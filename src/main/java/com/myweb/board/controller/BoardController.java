...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "write":
			...
			response.sendRedirect("board/board_wirte.jsp");
			break;
		case "regist":
			 ...
			 * 로직을 작성하지 않을 것이기 때문이다.(jsp는 단순히 보여지는 역할만 수행)
			 * 컨트롤러로 글 목록 요청이 다시 들어올 수 있게끔
			 * sendRedirect()를 사용하여 자동 목록 재 요청이 들어오게 하는 것이다.
			 */
		...
		case "list":
			 ...
			 * 응답이 바로 나가면서 request객체가 소멸해 버린다.
